...
public class InventoryManagerWife : MonoBehaviour
{
    public static InventoryManagerWife instance; // reference to the inventory manager script, making it a singleton type
    ...
    private InventoryItemControllerWife[] inventoryItems; // list of items in inventory of type inventoryItemControllerz
    ...
    {
        WifeInventorySaveData data = new WifeInventorySaveData();
        data.itemIDs = items.ConvertAll(item => item.id);

        string json = JsonUtility.ToJson(data);
        PlayerPrefs.SetString("SavedInventoryWife", json);
        PlayerPrefs.Save();
    }
    ...
    public void LoadInventory(ItemDatabase itemDatabase)
    {
        ...
        {
            string json = PlayerPrefs.GetString("SavedInventoryWife");
            WifeInventorySaveData data = JsonUtility.FromJson<WifeInventorySaveData>(json);
